...
from tokyotyrant import tt_power
...
class RecoverNumber(object):
    ...
    def _set_val(self, v):
        ...
        """
        tt_power.set_list(self.key, v)

    def _get_val(self):
        "値の取得。内容は _set_val() 参照"
        v = tt_power.get_list(self.key)
        if not v:
            return [-1,-1,0,0]
        return v
    ...
    def set_max_float(self, max_num, recover_term=None, cur_num=None):
        """
        最大値と回復間隔を設定？
        get_current()呼ぶ前には呼ばれる必要がある...
        最初に少なくとも一回はrecover_termは指定する必要がある...
        cur_num=None の場合、現在の値を取得して設定
        recover_term=None の場合、現在の値を取得して設定
        """
        last_up = int(time.time())
        if recover_term is None or cur_num is None:
            ...
            if cur_num is None:
                if old_all and old_all[0] != -1:
                    ...
                    cur_num = max_num
        v = [int(cur_num), int(max_num), recover_term, int(last_up)]
        self._set_val(v)
    ...
    def consume(self, consume_num, zero=False):
        ...
        # 体力消費の場合、
        if self.key.endswith(":current_power"):
            recover_term = StaticValues.RECOVER_POWER
        elif self.key.endswith(":current_love"):
            recover_term = StaticValues.RECOVER_LOVE
