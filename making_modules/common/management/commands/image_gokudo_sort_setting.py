...
#「納品」など被らない名前に
#python manage.py image_gokudo_sort_setting ~/Downloads/納品/ --settings=settings_local
...
image_size_list = [
    ...
    ('images', '240_zukan', 'GIF', 240, 240),
    ('images', '480', 'GIF', 480, 480),
    ('images', 'boss_240', 'GIF', 240, 240),
]
file_paths = []
        ...
        for file_path in file_paths:
            ...
            for category, dir_name, ext, width, height in image_size_list:
                ...
                if image_dir == dir_name and image.size[0] == width and image.size[1] == height and image.format == ext:
                    image_root_path = '%s/%s/gokudo/%s/' % (root_path, category, dir_name)
