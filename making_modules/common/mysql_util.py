...
def insert_or_update_many(model, objects, keys=None, using="default", limit=0):
    '''
...
def mysql_random(min, max, using="default"):
    '''
    mysqlサーバー経由で乱数を発生させる
    min:0, max:9の場合 0~ 9までの乱数を生成
    '''
    con = connections[using]
    cursor = con.cursor()
    cursor.execute("SELECT FLOOR(%d + (RAND() * %d));" %(min, (max - min + 1)))
    row = cursor.fetchone()
    return int(row[0])
